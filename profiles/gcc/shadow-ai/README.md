...
All users are auto-licensed with a Microsoft 365 G5 (GCC) SKU for mailbox provisioning.
...
- Communication Compliance has limited feature parity in GCC — some DSPM/advanced workflows may not be available.
- Insider Risk Management may have reduced template availability depending on GCC rollout stage.
- Conditional Access policies are excluded from the GCC config (manage separately via Entra admin center if needed).
- DLP enforcement parameters degrade gracefully — unsupported cmdlets are skipped with warnings.
